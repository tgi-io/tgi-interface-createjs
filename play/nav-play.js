...
(function () {
  ...
  function makeText(text, font, color, location, options) {
    ...
    color = color || '#000';
    return makeDisplayObject('TextObject', {
      text: text,
      font: font,
      color: color,
      location: location,
  ...
  var elvis = makeImage(res.assets.GuitarMan);
  imagePresentation.set('contents', [
    ...
    '',
    new tgi.Command({
      name: 'change image', type: 'Function', contents: function () {
        elvis._sourceElement.image = res.assets.MaleSwim.element;
      }
    }),
  ...
  var spriteContents = [
    ...
    new tgi.Command({
      name: 'change', type: 'Function', contents: function () {
        reel._sourceElement.spriteSheet = res.assets.Cards.face.spriteSheet;
      }
    }),
    reel
  ];
